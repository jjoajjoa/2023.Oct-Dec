...
router.post("/insert", function(req, res) {
    ...
    db.get().query(sql, [isbn], function(err, rows) {
        if(rows.length > 0) {
            ...
            sql = 'insert into books(title, authors, price, contents, publisher, image, isbn) values (?, ?, ?, ?, ?, ?, ?)';
            db.get().query(sql, [title, authors, price, contents, publisher, image, isbn], function(err, rows) {
                res.send("0");
            });
        };
    });
